...
import pymssql
import hashlib
import random, string, base64, os, re
...
def send_code(email):
    try:
        ...
        flash("Unvalid email address!")

def parametrize_string(string):
...
def login():
    ...
    return render_template('unsafe_login.html', user_logged_in=user_logged_in())
...
def search():
    ...
    if 1: #parametrize_string(user_name):
        sql = "select * from users where username='" + user_name + "'"
        cursor.execute(sql)
        users = cursor.fetchall()
        return redirect('/data/' + str(users[0]['Id']))
    else:
        flash("Please enter valid character!!!")
